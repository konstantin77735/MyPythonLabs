...
class KitchenFurnitureProduct(Product, Recyclable):
    def __init__(self, name, country_of_origin, purchase_price, retail_price, arrival_date_string, material, purpose,
                 length, height, width, utilized):
    ...
    def ask_discount(self):
        discount = int(input(f"Введите размер скидки (в процентах) для товара: {self.get_name()}, Категория: {self.get_purpose()}\n"
                             "Размер скидки не более 25%: "))
        if discount > 25:
            print("Скидка не может быть больше 25%!")
            discount = 25
